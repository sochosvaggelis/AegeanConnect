...
import { Briefcase, FileText, CheckCircle, XCircle, Clock, Eye, Plus } from 'lucide-react';
...
export default function Dashboard() {
    ...
    useEffect(() => {
        ...
        const load = async () => {
            if (me.role === 'hotel') {
                const [{ data: myJobs }, { data: allApps }] = await Promise.all([
                    supabase.from('jobs').select('*').eq('hotel_user_id', me.id).order('created_at', { ascending: false }).limit(50),
                    supabase.from('applications').select('*').eq('hotel_user_id', me.id).order('created_at', { ascending: false }).limit(50),
                ]);
                setJobs(myJobs || []);
                setApplications(allApps || []);
            } else {
                const { data: myApps } = await supabase.from('applications').select('*').eq('applicant_email', me.email).order('created_at', { ascending: false }).limit(50);
    ...
    return (
        <div style={{ background: '#eef4fd', minHeight: '100vh' }}>
            <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8">
                ...
                {isHotel && (
                    <div className="mb-8">
                        ...
                        <div className="space-y-3">
                            {jobs.length === 0 ? <p className="text-muted-foreground text-center py-8">{t('common_no_data')}</p> : jobs.map(job => (
                                <div key={job.id} className="bg-card rounded-xl border border-border/50 p-4 flex items-center justify-between gap-4">
                                    <div className="flex-1 min-w-0">
                                        <Link to={`/jobs/${job.id}`} className="font-medium text-foreground hover:text-primary transition-colors">{job.title}</Link>
                ...
                <div>
                    <h2 className="font-display text-xl font-bold text-foreground mb-4">{isHotel ? (lang === 'el' ? 'Αιτήσεις Υποψηφίων' : 'Candidate Applications') : t('dash_my_applications')}</h2>
                    <div className="space-y-3">
                            ...
                            <div key={app.id} className="bg-card rounded-xl border border-border/50 p-4">
                                <div className="flex items-start justify-between gap-4">
                                    <div className="flex-1 min-w-0">
                                        ...
                                        <p className="text-sm text-muted-foreground mt-0.5">{isHotel ? app.applicant_name : app.hotel_name}</p>
                                    ...
                                    <div className="flex flex-col items-end gap-2">
                                        ...
                                        {isHotel && app.status === 'pending' && (
                                            <div className="flex gap-1">
                                                <Button size="sm" variant="outline" className="h-7 text-xs rounded-lg" onClick={() => handleAppStatus(app.id, 'reviewed')}><Eye className="w-3 h-3 mr-1" />{t('dash_reviewed')}</Button>
                                                <Button size="sm" className="h-7 text-xs rounded-lg bg-green-600 hover:bg-green-700" onClick={() => handleAppStatus(app.id, 'accepted')}><CheckCircle className="w-3 h-3 mr-1" /></Button>
                                                <Button size="sm" variant="destructive" className="h-7 text-xs rounded-lg" onClick={() => handleAppStatus(app.id, 'rejected')}><XCircle className="w-3 h-3 mr-1" /></Button>
                                            </div>
                                        )}
                                        {isHotel && app.status === 'reviewed' && (
                                            <div className="flex gap-1">
                                                <Button size="sm" className="h-7 text-xs rounded-lg bg-green-600 hover:bg-green-700" onClick={() => handleAppStatus(app.id, 'accepted')}><CheckCircle className="w-3 h-3 mr-1" />{t('dash_accepted')}</Button>
                                                <Button size="sm" variant="destructive" className="h-7 text-xs rounded-lg" onClick={() => handleAppStatus(app.id, 'rejected')}><XCircle className="w-3 h-3 mr-1" /></Button>
                                            </div>
                                        )}
                                    </div>
